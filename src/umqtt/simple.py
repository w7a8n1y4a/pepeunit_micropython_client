...
class MQTTClient:
    ...
    def can_write(self):
        try:
            return self._poll(select.POLLOUT, 0)
        except Exception:
            return True
    ...
    def _sock_read_available(self, n):
        try:
    ...
    @staticmethod
    def _encode_varlen(length, buf, idx=1):
        while length > 0x7F:
            buf[idx] = (length & 0x7F) | 0x80
            length >>= 7
            idx += 1
        buf[idx] = length
        return idx
    ...
    def _rx_fill(self, need, blocking):
        ...
        while self._rx_len() < need:
            ...
            idx += 1
            utils._yield(idx, every=4)
        return True

    def _rx_ensure_from(self, idx, need, blocking):
        required = idx + need - self._rx_off
        if required <= 0:
            return True
        return self._rx_fill(required, blocking)

    def _rx_read_varlen(self, idx, blocking):
        rem = 0
        sh = 0
        loops = 0
        while True:
            if not self._rx_ensure_from(idx, 1, blocking):
                return None, None
            b = self._rx_buf[idx]
            idx += 1
            rem |= (b & 0x7F) << sh
            ...
            sh += 7
            loops += 1
            utils._yield(loops, every=4)

    def _rx_read(self, n, blocking=True):
        if not self._rx_fill(n, blocking):
            return None
        start = self._rx_off
        end = start + n
        out = bytes(self._rx_buf[start:end])
        self._rx_off = end
        self._rx_compact_if_needed()
        return out
    ...
    def connect(self, clean_session=True):
        ...
        i = self._encode_varlen(size, premsg, 1)
    ...
    def publish(self, topic, msg, retain=False, qos=0):
        ...
        if qos > 0:
            size += 2
        assert size < 2097152
        i = self._encode_varlen(size, pkt, 1)
            ...
            while 1:
                ...
                if op == 0x40:
                    ...
                    if pid == rcv_pid:
                        gc.collect()
                        return
                utils._yield(1, every=1, do_gc=False)
        elif qos == 2:
            assert 0
        gc.collect()
    ...
    def subscribe(self, topic, qos=0):
        ...
        struct.pack_into("!BH", pkt, 1, size, self.pid)
    ...
    def wait_msg(self, blocking=True):
        ...
        if self.socket_timeout is None:
            ...
            self._set_sock_timeout()
        if not self._rx_fill(1, blocking):
            return None
        ...
        rem, idx2 = self._rx_read_varlen(idx, blocking)
        if rem is None:
            return None
        if not self._rx_ensure_from(idx2, rem, blocking):
            return None
        ...
        if op & 6:
            ...
            idx += 2
            rem -= 2
        msg = bytes(self._rx_buf[idx:idx + rem])
        idx += rem
